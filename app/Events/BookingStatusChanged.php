...
class BookingStatusChanged implements ShouldBroadcastNow
{
    ...
    public function __construct(Booking $booking, string $status, string $message, array $data = [])
    {
    ...
    {
        $booking = $this->booking->load(['user', 'bookingContact', 'rideType', 'driver.driverProfile', 'driver.vehicles']);
        ...
        $driverAuthToken = '';
        if ($this->booking->driver_id) {
            $driverAuthToken = (string) $this->booking->driver_id;
        }
        ...
        $broadcastData = [
            ...
            'driver' => $this->formatDriverData($booking->driver),
            'pickup' => [
            ...
            'event_type' => $this->getEventType($this->status),
            'status' => $this->status,
            'message' => $this->message,
            'data' => $this->data,
    ...
    private function formatDriverData($driver): array
    {
        ...
        return [
            ...
            'vehicle' => [
                ...
                'color' => $vehicle->color ?? '',
                'number_plate' => $vehicle->registration_number ?? '',
            ...
            'current_location' => [
                ...
                'recorded_at' => $currentLocation ? $currentLocation->recorded_at->toISOString() : '',
            ],
        ];
    }
